...
std::string getTrackerPatch(TrackerStruct &tracker) {
	cv::Point2d cv_corners[4];
	mtf::utils::Corners(tracker.getRegion()).points(cv_corners);
    std::stringstream patch;
    patch << "(" << cv_corners[0].x << "," << cv_corners[0].y << ")";
    patch << "(" << cv_corners[1].x << "," << cv_corners[1].y << ")";
    patch << "(" << cv_corners[2].x << "," << cv_corners[2].y << ")";
    patch << "(" << cv_corners[3].x << "," << cv_corners[3].y << ")";

	cv::Point center_point = getPatchCenter(cv_corners);
    patch << "(" << center_point.x << "," << center_point.y << ")";
    std::string final_patch = patch.str();
...
void updateTrackers() {
    ...
    for(std::vector<TrackerStruct>::iterator tracker = trackers.begin(); 
        ...
        std::string tracker_patch = getTrackerPatch(*tracker);
...
int main(int argc, char *argv[]) {
    ...
    tracker_pub = nh_.advertise<std::string>("patch_tracker", 1);
